module soga
    ...
    subroutine calc_fitness(this, population) ! TODO
        ...
        case default
            write(0, "(3a)") "Error: unknown fitness_type '", this%fitness_type, "'"
            write(0, "(a)") "in subroutine 'calc_fitness'"
            stop 1
        end select
            ...
            outer: do j = 1, size(this%history, dim=2)
                ...
                inner: do i = 1, this%pop_size
                    associate (h => this%history(i, j))
                        ...
                        write(unit, "(',',es15.8,2(',',i0))") h%fitness, h%indiv%parents_id
                    end associate
                end do inner
                ! end if
            end do outer
