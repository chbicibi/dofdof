module moead
    use util, only: distance, filled, sort
    use interface
    use individual
    use problem
    use moead_unit
    ...
    subroutine init_weight(this)
        ...
        case default
            write(0, "(a,i0,a)") "Error: wrong number of objectives (given ", &
                                 this%num_obj, ", expected 2 or 3)"
            write(0, "(a)") "in subroutine 'init_weight'"
            stop 1
        end select
    ...
    subroutine calc_fitness(this, population)
        ...
        do j = 1, pop_size
            associate (table=>population(j)%table)
                objective = [(this%scalar_value(population(table(i))%indiv, j), &
                              i = 1, this%table_size)]
